...
1.  **Appointment Reminders**: It sends WhatsApp reminders for confirmed appointments. This includes 24-hour and 1-hour reminders, gated by the `canUseFeature` function to ensure the professional's plan supports reminders. Reminders are deferred if the client is actively engaged in an AI conversation, as determined by `isClientActive`.
