...
*(Note: The implementation details for the `src/app/api/whatsapp/webhook` endpoint are not available in the provided code snippet.)*
...
The agent uses Groq's LLM with a predefined set of tools to interact with the database and manage the booking process.

### Tool Definitions

The `TOOLS` array in `src/lib/ai-intake.ts` defines the functions the Groq LLM can call. These include:
...
1.  **Professional Lookup**: It first retrieves the professional's details from the `professionals` table.
2.  **Date Validation & Day of Week**: The input `dateStr` is validated, and the day of the week is determined in the "America/Sao_Paulo" timezone.
3.  **Availability Rules**: It queries the `availability` table to find the professional's working hours for that specific day.
4.  **Existing Appointments**: It fetches existing appointments for the given date from the `appointments` table, excluding `rejected` or `cancelled` statuses.
5.  **Slot Generation**: It iterates through the availability rules, generating potential slots based on the `sessionDuration`. Each potential slot is checked against existing appointments to prevent double-bookings and ensures only future slots are offered.

Brazilian time zone handling is explicit, with `saoPauloDate` and `formatSaoPauloTime`/`formatSaoPauloDate` helpers ensuring consistency (Brazil abolished DST in 2019, so São Paulo is consistently UTC-3).

### `bookAppointment` Implementation

The `bookAppointment` function in `src/lib/ai-intake.ts` is designed to create a new appointment in the database. It takes the professional ID, date, time, client name, and client phone as arguments.

*(Note: The provided code snippet for `bookAppointment` is truncated, so its full implementation details are not available.)*

### `getProfessionalInfo`

This tool is defined to retrieve professional information but its implementation is not included in the provided `src/lib/ai-intake.ts` snippet.
...
Conversation state is stored in Redis, keyed by `conv:{professionalId}:{clientPhone}`. Each `ConversationState` object includes:
...
Conversations have a `TTL` of 30 minutes, after which they expire from Redis.
...
The `setOutboundContext` and `getOutboundContext` functions in `src/lib/conversation.ts` store and retrieve the context of the last transactional message sent to a client. This `OutboundContext` includes the `professionalId` and optionally an `appointmentId`. It has an `OUTBOUND_TTL` of 2 hours. This allows inbound replies like "I need to reschedule" to be automatically routed to the correct professional and appointment without requiring the client to re-specify details.

## Why this shape

The design of the WhatsApp AI intake agent prioritizes a robust, stateful interaction model. The two-pass LLM pattern allows for explicit tool-calling, ensuring that complex actions like booking appointments are handled systematically. Storing conversation state in Redis (`src/lib/conversation.ts`) enables the AI to maintain context across messages, providing a more natural and efficient user experience. The explicit handling of São Paulo time zones in `src/lib/ai-intake.ts` ensures accurate scheduling without relying on server-side timezone configurations.

## Known gaps

*   **Incomplete `bookAppointment` implementation**: The provided `src/lib/ai-intake.ts` snippet for the `bookAppointment` function is truncated, meaning the full logic for persisting the appointment and handling subsequent notifications is not visible.
