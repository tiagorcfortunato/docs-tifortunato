...
2. **Identity** — `getUser()` returns the authenticated Supabase user.
3. **Idempotency** — if a `professionals` row already exists for this `user.id`, return it without recreating.
4. **Slug generation** — `generateUniqueSlug(toSlug(body.name))`:
   - `toSlug()` normalizes NFD → strips diacritics → lowercase → dash-separated → drops non-alphanumerics
   - `generateUniqueSlug()` tries the base; on collision, appends `-2`, `-3`, `-4`, … with one DB round-trip per iteration
5. **Insert professional** — one row.
6. **Insert availability** — one row per weekday window.
7. **Set user metadata** — `{ type: "professional" }` so future logins route to the dashboard.
...
Captured here rather than pretended away — backlog items, not bugs blocking today's users:

- **No Zod validation on the onboarding body.** Unlike `/api/booking` which uses a Zod schema, `/api/onboarding` trusts the client-side form. A malformed request (`sessionPrice` as string, `sessionDuration` negative) would be written to Postgres as-is. Hardening item — wrap the body in a Zod schema before the first real user.
- **No reserved-slug guard.** `toSlug("Dashboard")` → `dashboard` is accepted unchanged. If a professional's name slugs to a top-level route (`dashboard`, `login`, `register`, `api`, `explore`, `c`, `p`, `onboarding`, `privacidade`, `termos`), `/p/dashboard` would collide with the Next.js route. Hardening item — add a reserved-name blocklist to `generateUniqueSlug()`.
- **Unbounded slug-collision loop.** One DB query per retry. Fine at MVP scale; pathological only if thousands of pros collide on the same base name.
