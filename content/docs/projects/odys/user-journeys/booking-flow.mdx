...
The full action set on PATCH `/api/appointments/[id]` is: `confirm`, `reject`, `cancel`, `paid`, `complete`, `no_show`. `paid` / `complete` / `no_show` are pro-only (authentication + ownership gate).
...
**Upgrade path** if real contention emerges: either wrap select+insert in `db.transaction(...)` with `SERIALIZABLE` isolation, or add a Postgres `EXCLUSION CONSTRAINT` using `tstzrange(starts_at, ends_at)` with `&&` operator. The `generateSlots()` function stays unchanged — it's UX, not authorization.
